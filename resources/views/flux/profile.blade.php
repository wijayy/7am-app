...
@php
    ...
    $iconClasses = Flux::classes(
        'text-gray-400 dark:text-white/80 group-hover:text-gray-800 dark:group-hover:text-white',
    ...
    ->add('p-1 hover:bg-gray-800/5 dark:hover:bg-white/10');
@endphp
...
<button type="button" {{ $attributes->class($classes) }} data-flux-profile>
    <div class="shrink-0">
        ...
        <?php else: ?>
        <flux:avatar
            :attributes="Flux::attributesAfter('avatar:', $attributes, ['src' => $avatar, 'size' => 'sm', 'circle' => $circle,
    ...
    <?php if ($name): ?>
    <span
        class="mx-2 text-sm text-gray-500 dark:text-white/80 group-hover:text-gray-800 dark:group-hover:text-white font-medium truncate">
        {{ $name }}
    </span>
    <?php endif; ?>
